...
//  KSR Cranes App
//
//  ViewModels for Chef project management - FIXED with robust error handling
//
...
// MARK: - ProjectDetailViewModel - ✅ COMPLETELY FIXED VERSION
...
class ProjectDetailViewModel: ObservableObject {
    ...
    // ✅ FIXED: Handle both simple Project and complex ChefProjectDetail responses
    func loadProjectDetail(projectId: Int) {
        ...
        ChefProjectsAPIService.shared.makeRequest(
            endpoint: "/api/app/chef/projects/\(projectId)?include_customer=true&include_billing=true&include_stats=true",
            method: "GET",
            body: Optional<String>.none
        )
        .tryMap { data -> Project in
            #if DEBUG
            if let jsonString = String(data: data, encoding: .utf8) {
                print("🔍 [ProjectDetailViewModel] Raw API response:")
                print(jsonString.prefix(500)) // Show first 500 chars
            }
            #endif
            
            let decoder = BaseAPIService.createAPIDecoder()
            
            // Strategy 1: Try to decode as ChefProjectDetail first
            do {
                let detail = try decoder.decode(ChefProjectDetail.self, from: data)
                #if DEBUG
                print("✅ [ProjectDetailViewModel] Decoded as ChefProjectDetail")
                #endif
                ...
                #if DEBUG
                print("⚠️ [ProjectDetailViewModel] ChefProjectDetail decoding failed: \(error)")
                print("   Trying direct Project decoding...")
                #endif
                
                // Strategy 2: Try to decode as simple Project (which seems to be what your API returns)
                do {
                    let project = try decoder.decode(Project.self, from: data)
                    #if DEBUG
                    print("✅ [ProjectDetailViewModel] Decoded as simple Project: \(project.title)")
                    #endif
                    return project
                } catch {
                    #if DEBUG
                    print("❌ [ProjectDetailViewModel] Both decoding strategies failed")
                    print("   ChefProjectDetail error: \(error)")
                    #endif
                    throw error
                }
            }
        ...
        .sink(
            receiveCompletion: { [weak self] completion in
                ...
                switch completion {
                case .failure(let error):
                    let errorMsg = "Failed to load project details: \(error.localizedDescription)"
                    #if DEBUG
                    print("❌ [ProjectDetailViewModel] \(errorMsg)")
                    #endif
                    // Don't show error for this since we can work with the project we already have
    ...
    /// Use this method if your API always returns a simple Project object for project details
    func loadProjectDetailSimple(projectId: Int) {
        isLoadingDetail = true
        
        #if DEBUG
        print("[ProjectDetailViewModel] Loading simple project detail for ID: \(projectId)")
        #endif
        
        // Use the existing fetchProject method but handle simple Project response
        ChefProjectsAPIService.shared.makeRequest(
            endpoint: "/api/app/chef/projects/\(projectId)?include_customer=true",
            method: "GET",
            body: Optional<String>.none
        )
        .decode(type: Project.self, decoder: BaseAPIService.createAPIDecoder())
        .receive(on: DispatchQueue.main)
        .sink(
            receiveCompletion: { [weak self] completion in
                self?.isLoadingDetail = false
                if case .failure(let error) = completion {
                    #if DEBUG
                    print("⚠️ [ProjectDetailViewModel] Simple project loading failed: \(error.localizedDescription)")
                    #endif
                    // Don't show error since we can work with the project we already have
                }
            },
            receiveValue: { [weak self] project in
                self?.project = project
                #if DEBUG
                print("✅ [ProjectDetailViewModel] Loaded simple project: \(project.title)")
                #endif
            }
        )
        .store(in: &cancellables)
    ...
    // ✅ FIXED: Enhanced loadTasks method with robust error handling
    func loadTasks(for projectId: Int) {
        isLoadingTasks = true
        tasksError = nil
        
        #if DEBUG
        print("🔄 [ProjectDetailViewModel] Loading tasks for project: \(projectId)")
        #endif
        ...
        .sink(
            receiveCompletion: { [weak self] completion in
                ...
                case .failure(let error):
                    #if DEBUG
                    print("❌ [ProjectDetailViewModel] Tasks loading failed: \(error)")
                    #endif
                    self?.tasksError = error.localizedDescription
                    
                case .finished:
                    #if DEBUG
                    print("✅ [ProjectDetailViewModel] Tasks loading completed successfully")
                    #endif
                }
            },
            receiveValue: { [weak self] response in
                #if DEBUG
                print("✅ [ProjectDetailViewModel] Received \(response.tasks.count) tasks")
    ...
    // ✅ ENHANCED: Better error handling with task-specific error management
    private func showError(_ message: String) {
        errorMessage = message
        showError = true
        
        // Only set tasksError if this is specifically a task-related error
        if message.contains("tasks") || message.contains("task") {
            tasksError = message
        }
