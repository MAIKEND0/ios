    ...
    for (const worker of workers) {
      ...
      for (const date of dateRange) {
        ...
        const tasksForDay = taskAssignments.filter(assignment => {
          ...
          const taskStart = null; // start_date not available on Tasks from TaskAssignments
        ...
        const estimatedHoursForDay = tasksForDay.reduce((sum, assignment) => {
          return sum + 8; // Default 8 hours per task assignment
        }, 0);
        ...
        dailyAvailability[dateKey] = {
          ...
          tasks: tasksForDay.map(assignment => ({
            ...
            taskName: assignment.Tasks?.title || '',
            projectName: assignment.Tasks?.Projects?.title || '',
            hours: 8, // Default task duration
