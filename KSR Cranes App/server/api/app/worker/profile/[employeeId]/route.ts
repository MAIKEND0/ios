// src/app/api/app/worker/profile/[workerId]/avatar/route.ts
...
async function authenticate(req: NextRequest) {
  ...
  try {
    ...
    // KLUCZOWE: Sprawdź rolę worker'a
...
export async function POST(
  ...
  try {
    ...
    if (authenticatedId.toString() !== workerId) {
    ...
    const s3Key = `profiles/${workerId}/${filename}`;
    ...
    const existingWorker = await prisma.employees.findUnique({
      where: { employee_id: parseInt(workerId) },
      select: { profilePictureUrl: true, role: true }
    });
    ...
    if (existingWorker.role !== "arbejder") {
      return NextResponse.json(
        { error: "Unauthorized: Not a worker account" },
        { status: 403 }
      );
    }
    ...
    // Generate public URL
    const profileImageUrl = getEmployeeProfileImageUrl(workerId, filename);
    ...
    console.log(`[API] Profile picture updated for worker ${workerId}: ${profileImageUrl}`);
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Error deleting worker profile picture:", error);
    return NextResponse.json(
